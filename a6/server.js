...
const { recursoEstatico, sirvoRecursoEstatico } = require('./static.js')
...
function geraForm2Aluno(aluno,d) {
    return `
    <html>
        <head>
        ...
        <body>
            ...
            <form class="w3-container" action="/alunos" method="POST">
                <label class="w3-text-teal"><b>Nome</b></label>
                <input class="w3-input w3-border w3-light-grey" type="text" name="nome"  value=${aluno.nome}>
          
                <label class="w3-text-teal"><b>Número / Identificador</b></label>
                <input class="w3-input w3-border w3-light-grey" type="text" name="id" value=${aluno.id} disabled>

                <label class="w3-text-teal"><b>Curso</b></label>
                <input class="w3-input w3-border w3-light-grey" type="text" name="curso" value=${aluno.curso}>

                <label class="w3-text-teal"><b>Link para o repositório no Git</b></label>
                <input class="w3-input w3-border w3-light-grey" type="text" name="git" value=${aluno.git}>
    ...
    switch (req.method) {
        case "GET":
            ...
            // GET /alunos/:id --------------------------------------------------------------------
                ...
                axios.get("http://localhost:3000/alunos/" + idAluno)
                    .then(response => {
                        ...
                        res.writeHead(200, { 'Content-Type': 'text/html;charset=utf-8' })
                        res.write(geraPagAluno(aluno, d))
                        res.end()
                        // Add code to render page with the student record
                    })
                    .catch(function (erro) {
                        res.writeHead(200, { 'Content-Type': 'text/html;charset=utf-8' })
                        res.write("<p>Não foi possível obter o registo do aluno...")
                        res.end()
                    })
            }
            // GET /alunos/registo --------------------------------------------------------------------
            ...
            // GET /alunos/:id/edit --------------------------------------------------------------------
                ...
                const idAluno=req.url.split('/')[2]
                axios.get("http://localhost:3000/alunos/" + idAluno)
                    .then(response => {
                        let aluno = response.data
                        res.writeHead(200, { 'Content-Type': 'text/html;charset=utf-8' })
                        res.write(geraForm2Aluno(aluno, d))
                        res.end()
                        // Add code to render page with the student record
                    })
                    .catch(function (erro) {
            ...
            break
        case "POST"&&/\/alunos\/(A|PG)[0-9]/.test(req.url):
            ...
            break
        case "POST"&&/\/alunos\/(A|PG)[0-9]\/edit/.test(req.url):
            
                recuperaInfo(req, resultado => {
                    axios.put('http://localhost:3000/alunos', resultado)
                        .then(resp => {
                            res.writeHead(200, { 'Content-Type': 'text/html;charset=utf-8' })
                            res.write(geraPostConfirm(resp.data, d))
                            res.end()
